...
class ParserArtistCommand extends Command
{
    ...
    private function getArtist(int $artist_id): Artist|bool
    {
        ...
        // Засыпаем на несколько секунд, когда получаем новый файл с артистом.
        // Может быть, поможет от антипарсеров, но не уверен, что это сработает
        sleep(rand(1,10));
    ...
    private function addTrackToDatabase(object $track_data)
    {
        ...
        foreach ($track_data->artists as $artist) {
            ...
            // то пропускаем трек. В данном случае можно было бы сделать некую
            // очередь из треков, которые обработались с ошибкой, чтобы можно
            // было их в другое время отдельно обработать.
